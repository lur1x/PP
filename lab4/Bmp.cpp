...
Bitmap::Bitmap(const char *path)
    : m_bmpInfo(), m_pixels(nullptr)
{
    std::ifstream file(path, std::ios::in | std::ios::binary);

    if (!file)
    {
        throw std::runtime_error("Не удалось открыть файл: " + std::string(path));
    }
    ...
    {
        throw std::runtime_error("Неверный BMP файл");
    }

    int32_t height = m_bmpInfo.bih.biHeight;
    if (height == 0)
    {
        throw std::runtime_error("Некорректный BMP: высота равна нулю");
    }
    ...
    {
        throw std::runtime_error("Поддерживаются только 24-битные и 32-битные BMP");
    }
    ...
    if (file.gcount() != pixelDataSize)
    {
        delete[] m_pixels;
        m_pixels = nullptr;
        throw std::runtime_error("Ошибка чтения данных пикселей");
    }
...
void Bitmap::Save(const char *path, uint16_t bit_count)
{
    ...
    {
        throw std::runtime_error("Поддерживается вывод только 24-битных и 32-битных изображений");
    }
